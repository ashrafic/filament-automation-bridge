<div class="space-y-6" x-on:click.stop="">
    @if($record->payload)
        <x-filament::section>
            ...
            <pre x-data="{ copied: false }" class="relative max-h-64 overflow-auto rounded-lg bg-gray-900 p-4 pr-20 text-xs leading-relaxed whitespace-pre-wrap break-all text-green-400 dark:bg-gray-950">
                <button
                    type="button"
                    x-on:click="navigator.clipboard.writeText($refs.content.textContent).then(() => { copied = true; setTimeout(() => copied = false, 2000) }).catch(() => {})"
                    class="absolute top-2 right-2 z-10 flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium bg-gray-700 text-gray-200 hover:bg-gray-600 transition"
                >
                ...
                </button><span x-ref="content">{{ json_encode($record->payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</span></pre>
        </x-filament::section>
    @endif
        ...
        <x-filament::section>
            ...
            <pre x-data="{ copied: false }" class="relative max-h-64 overflow-auto rounded-lg bg-gray-900 p-4 pr-20 text-xs leading-relaxed whitespace-pre-wrap break-all text-green-400 dark:bg-gray-950">
                ...
                >
                    ...
                    <svg x-show="copied" x-cloak xmlns="http://www.w3.org/2000/svg" class="size-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                    <span x-text="copied ? 'Copied' : 'Copy'"></span>
                </button><span x-ref="content">{{ json_encode($record->headers, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</span></pre>
        </x-filament::section>
    @endif
        ...
        <x-filament::section>
            ...
            <pre x-data="{ copied: false }" class="relative max-h-64 overflow-auto rounded-lg bg-gray-900 p-4 pr-20 text-xs leading-relaxed whitespace-pre-wrap break-all text-green-400 dark:bg-gray-950">
                ...
                >
                    ...
                    <svg x-show="copied" x-cloak xmlns="http://www.w3.org/2000/svg" class="size-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                    <span x-text="copied ? 'Copied' : 'Copy'"></span>
                </button><span x-ref="content">{{ is_string($record->response_body) ? $record->response_body : json_encode($record->response_body, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</span></pre>
        </x-filament::section>
    @endif
        ...
        <x-filament::section>
            ...
            <pre x-data="{ copied: false }" class="relative max-h-64 overflow-auto rounded-lg bg-gray-900 p-4 pr-20 text-xs leading-relaxed whitespace-pre-wrap break-all text-green-400 dark:bg-gray-950">
                ...
                >
                    ...
                    <svg x-show="copied" x-cloak xmlns="http://www.w3.org/2000/svg" class="size-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                    <span x-text="copied ? 'Copied' : 'Copy'"></span>
                </button><span x-ref="content">{{ json_encode($record->response_headers, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</span></pre>
        </x-filament::section>
    @endif
